package Ingress::Field;
{
  $Ingress::Field::VERSION = '0.20';
}
use strict;
use warnings;
use Geo::Distance;
...
Ingress::Field - Calculate Distances and Closest Locations

=head1 SYNOPSIS

  use Ingress::Field;
  my $geo = new Ingress::Field;
  $geo->formula('hsin');
...
near future Ingress::Field will become a lightweight wrapper around GIS::Distance so that
legacy code benefits from fixes to GIS::Distance through the old Ingress::Field API.  For
any new developement I suggest that you look in to GIS::Distance.

=head1 STABILITY

The interface to Ingress::Field is fairly stable nowadays.  If this changes it 
will be noted here.
...
are defined in a Ingress::Field object.  You can add units with reg_unit() or create some default 
...
=head2 new
  ...
  my $geo = new Ingress::Field;
  my $geo = new Ingress::Field( no_units=>1 );

Returns a blessed Ingress::Field object.  The new constructor accepts one optional 
argument.
...
sub length {
    ...
    my $linksrclatitude=$fieldhash->{edge}{originPortalLocation}{latE6}/1000000;
    my $linksrclongitude=$fieldhash->{edge}{originPortalLocation}{lngE6}/1000000;
    my $linkdstlatitude=$fieldhash->{edge}{destinationPortalLocation}{latE6}/1000000;
    my $linkdstlongitude=$fieldhash->{edge}{destinationPortalLocation}{lngE6}/1000000;
...
sub mu {
    my($self,$fieldhash) = @_;
    return $fieldhash->{entityScore}{entityScore};
}
sub sq {
    my($self,$fieldhash) = @_;
    #p=`echo "($a+$b+$c)/2"|bc`
    #echo link length $a $b $c
    #square=`echo "sqrt($p*($p-$a)*($p-$b)*($p-$c))"|bc`
    
    return 666;
}
sub team {
    my($self,$fieldhash) = @_;
    return $fieldhash->{controllingTeam}{team};
}
sub vertexalat {
    my($self,$fieldhash) = @_;
    return $fieldhash->{capturedRegion}{vertexA}{location}{latE6}/1000000;
}
sub vertexalon {
      my($self,$fieldhash) = @_;
      return $fieldhash->{capturedRegion}{vertexA}{location}{lngE6}/1000000;
}
sub vertexa {
    my($self,$fieldhash) = @_;
    my $lat=$fieldhash->{capturedRegion}{vertexA}{location}{latE6}/1000000;
    my $lon=$fieldhash->{capturedRegion}{vertexA}{location}{lngE6}/1000000;
    return $lat.",".$lon;
}

sub vertexblat {
    my($self,$fieldhash) = @_;
    return $fieldhash->{capturedRegion}{vertexB}{location}{latE6}/1000000;
}
sub vertexblon {
      my($self,$fieldhash) = @_;
      return $fieldhash->{capturedRegion}{vertexB}{location}{lngE6}/1000000;
}
sub vertexb {
    my($self,$fieldhash) = @_;
    my $lat=$fieldhash->{capturedRegion}{vertexB}{location}{latE6}/1000000;
    my $lon=$fieldhash->{capturedRegion}{vertexB}{location}{lngE6}/1000000;
    return $lat.",".$lon;
}

sub vertexclat {
    my($self,$fieldhash) = @_;
    return $fieldhash->{capturedRegion}{vertexC}{location}{latE6}/1000000;
}
sub vertexclon {
      my($self,$fieldhash) = @_;
      return $fieldhash->{capturedRegion}{vertexC}{location}{lngE6}/1000000;
}
sub vertexc {
    my($self,$fieldhash) = @_;
    my $lat=$fieldhash->{capturedRegion}{vertexC}{location}{latE6}/1000000;
    my $lon=$fieldhash->{capturedRegion}{vertexC}{location}{lngE6}/1000000;
    return $lat.",".$lon;
}

sub ctime {
    my($self,$fieldhash) = @_;
    return $fieldhash->{creator}{creationTime};
}
sub cguid {
    my($self,$fieldhash) = @_;
    return $fieldhash->{creator}{creatorGuid};
}
...
Currently Ingress::Field only has spherical and flat type formulas.  
...
The hsin formula is the new standard formula for Ingress::Field because 
...
Ingress::Field (ending at version 0.06) and was wrongly called the "gcd" formula.
...
If L<Ingress::Field::XS> is installed, this module will use it. You can
