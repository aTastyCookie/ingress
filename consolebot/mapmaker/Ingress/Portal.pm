package Ingress::Portal;
{
  $Ingress::Portal::VERSION = '0.20';
}
use strict;
use warnings;
use Geo::Distance;
...
Ingress::Portal - Calculate Distances and Closest Locations

=head1 SYNOPSIS

  use Ingress::Portal;
  my $geo = new Ingress::Portal;
  $geo->formula('hsin');
...
near future Ingress::Portal will become a lightweight wrapper around GIS::Distance so that
legacy code benefits from fixes to GIS::Distance through the old Ingress::Portal API.  For
any new developement I suggest that you look in to GIS::Distance.

=head1 STABILITY

The interface to Ingress::Portal is fairly stable nowadays.  If this changes it 
will be noted here.
...
are defined in a Ingress::Portal object.  You can add units with reg_unit() or create some default 
...
=head2 new
  ...
  my $geo = new Ingress::Portal;
  my $geo = new Ingress::Portal( no_units=>1 );

Returns a blessed Ingress::Portal object.  The new constructor accepts one optional 
argument.
...
Currently Ingress::Portal only has spherical and flat type formulas.  
...
The hsin formula is the new standard formula for Ingress::Portal because 
...
Ingress::Portal (ending at version 0.06) and was wrongly called the "gcd" formula.
...
If L<Ingress::Portal::XS> is installed, this module will use it. You can
