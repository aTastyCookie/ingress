...
#  print "$file \n";
  ...
  foreach my $rec (@{$json->{gameBasket}{gameEntities}}) {
    ...
    my $capturedTime=1;
    my $capturedTime=`mysql ingresscaptures -e "select msgts*1000 from syscomm where portalgid='$guid' order by msgts desc  limit 1;"|grep -v msgts`;
    chomp $capturedTime;
#    if ($capturedTime eq "") {
#     my $capturedTime=`mysql ingress -e "select msgts*1000 from syscomm where portalgid='$guid' order by msgts desc  limit 1;"|grep -v msgts`;
#   }
    $gameentities->{$guid}{entity}{captured}{capturedTime}=$capturedTime;
  }
}
my %finaljson;
