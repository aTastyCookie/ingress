...
echo "eating xm.."
java -jar bin/S2Geometry.jar $LAT $LON   0.02 0.02 > cells.txt
...
./bin/getObjectsInCellsa.1.pl response/getObjectsInCells cells.txt $HEXLATE6 $HEXLONE6 |gzip  > payload.gz
./bin/postapi.sh getObjectsInCells
guids=`cat response/getObjectsInCells|json_xs |grep -iE '"[a-z0-9]{32}.6"'|wc -l`
