package Ingress::Item;
{
  $Ingress::Item::VERSION = '0.20';
}
use strict;
#use warnings;
=head1 NAME

Ingress::Item - Calculate Distances and Closest Locations

=head1 SYNOPSIS

  use Ingress::Item;
  my $geo = new Ingress::Item;
  $geo->formula('hsin');
...
near future Ingress::Item will become a lightweight wrapper around GIS::Distance so that
legacy code benefits from fixes to GIS::Distance through the old Ingress::Item API.  For
any new developement I suggest that you look in to GIS::Distance.

=head1 STABILITY

The interface to Ingress::Item is fairly stable nowadays.  If this changes it 
will be noted here.
...
are defined in a Ingress::Item object.  You can add units with reg_unit() or create some default 
...
=head2 new
  ...
  my $geo = new Ingress::Item;
  my $geo = new Ingress::Item( no_units=>1 );

Returns a blessed Ingress::Item object.  The new constructor accepts one optional 
argument.
...
Currently Ingress::Item only has spherical and flat type formulas.  
...
The hsin formula is the new standard formula for Ingress::Item because 
...
Ingress::Item (ending at version 0.06) and was wrongly called the "gcd" formula.
...
If L<Ingress::Item::XS> is installed, this module will use it. You can
